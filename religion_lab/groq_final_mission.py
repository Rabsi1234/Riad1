...
# مفتاح Groq "اللفة الطويلة" الشغال
...
prompt_template = """
أنت خبير تربوي في المنهاج الأردني. قم بتحويل النص التالي لشرح "نانو" مكثف ومفصل:
1. تلخيص مركز: أهم الأفكار.
2. شرح تفصيلي وعميق: تبسيط المفاهيم.
3. أمثلة توضيحية وتطبيقية.
4. بنك أسئلة (سؤال وجواب) متوقعة ومقترحة للامتحان.
النص: {content}
"""
...
def call_groq(content):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"    ❌ خطأ API: {response.status_code} - {response.text[:50]}")
            return None
...
for start, end, folder in lessons:
    ...
    for p in range(start, end + 1):
        ...
        print(f"  🧠 تحليل صفحة {p} باستخدام Llama 3.3...")
        ...
        if result:
            ...
            print(f"  🛑 فشلت معالجة الصفحة {p}.")

print("✨ انتهت المهمة بنجاح! جميع دروس التربية الإسلامية جاهزة الآن.")
